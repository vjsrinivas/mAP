...
parser.add_argument('-np', '--no-plot', help='no plot is shown.',
                    action='store_true')
parser.add_argument('-q', '--quiet', help='minimalistic console output.'
                    , action='store_true')
...
def draw_text_in_image(
    ...
    (text_width, _) = cv2.getTextSize(text, font, fontScale,
            lineType)[0]
...
for (class_index, class_name) in enumerate(gt_classes):
    ...
    bounding_boxes.sort(key=lambda x: float(x['confidence']),
                        reverse=True)
    with open(TEMP_FILES_PATH + '/' + class_name + '_dr.json', 'w') as \
        outfile:
        json.dump(bounding_boxes, outfile)
    ...
    for (class_index, class_name) in enumerate(gt_classes):
        ...
        for (idx, detection) in enumerate(dr_data):
            ...
            if show_animation:
                ...
                (img, line_width) = draw_text_in_image(img, text,
                        (margin + line_width, v_pos), light_blue,
                        line_width)
                if ovmax != -1:
                    color = light_red
                ...
                if ovmax > 0:  # if there is intersections between the bounding-boxes
                    bbgt = [int(round(float(x))) for x in
                            gt_match['bbox'].split()]
                    cv2.rectangle(img, (bbgt[0], bbgt[1]), (bbgt[2],
                                  bbgt[3]), light_blue, 2)
                ...
                cv2.rectangle(img, (bb[0], bb[1]), (bb[2], bb[3]),
                              color, 2)
                cv2.rectangle(img_cumulative, (bb[0], bb[1]), (bb[2],
                              bb[3]), color, 2)
                cv2.putText(
                    img_cumulative,
                    class_name,
                ...
                output_img_path = results_files_path \
                    + '/images/detections_one_by_one/' + class_name \
...
''')
        ...
        if draw_plot:
            ...
            fig.savefig(results_files_path + '/classes/' + class_name
                        + '.png')
